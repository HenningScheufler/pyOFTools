...
def test_csv_values_match_reference(run_reset_case, change_test_dir):
    ...
    assert np.isclose(first_row["time"], 0.00119, atol=1e-4), "First timestep should be around 0.00119"
    ...
    first_time_group0 = df_mass[(df_mass["time"] == df_mass["time"].min()) & (df_mass["group"] == 0)]
    assert len(first_time_group0) == 1, "Should have exactly one entry for group 0 at first timestep"
    assert first_time_group0["rho_volIntegrate"].values[0] == 0.0, "Group 0 should have zero mass initially"
