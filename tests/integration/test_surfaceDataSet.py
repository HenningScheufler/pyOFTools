...
from pybFoam import (
    ...
    dictionary, Word, vector, wordList, scalarField, vectorField,
    boolList, labelList
)
...
def test_create_simple_scalar_surface_dataset(change_to_cube_dir):
    ...
    plane_config = SampledCuttingPlaneConfig(
        point=[0.0, 0.0, 0.0],
        normal=[0.0, 0.0, 1.0]
    )
    ...
    plane_surface = sampledSurface.New(
        Word("testCuttingPlane"),
        mesh,
        plane_dict
    )
...
def test_create_simple_scalar_surface_dataset_workflow(change_to_cube_dir):
    ...
    surface_dataset = SurfaceDataSet(
        name="test_scalar_surface",
        field=field,
        geometry=SampledSurfaceAdapter(plane_surface)
    )
    
    w = WorkFlow(
        initial_dataset=surface_dataset
    ).then(Sum())
