    ...
    with patch("pyOFTools.sets.sampledSet") as mock_sampledSet:
        with patch("pyOFTools.sets.meshSearch", return_value=mock_mesh_search):
            ...
            result = create_uniform_set(
                mesh=mock_mesh, name="line1", start=[0, 0, 0], end=[1, 0, 0], n_points=10
            )
...
def test_create_cloud_set_basic(mock_mesh, mock_mesh_search):
    ...
    with patch("pyOFTools.sets.sampledSet") as mock_sampledSet:
        with patch("pyOFTools.sets.meshSearch", return_value=mock_mesh_search):
            ...
            result = create_cloud_set(mesh=mock_mesh, name="cloud1", points=points)
        ...
        with patch("pyOFTools.sets.meshSearch", return_value=mock_mesh_search):
            with patch("pyOFTools.sets.Word") as mock_word:
                ...
                result = create_cloud_set(
                    mesh=mock_mesh, name="cloud1", points=points, axis="distance"
                )
    ...
    with patch("pyOFTools.sets.sampledSet") as mock_sampledSet:
        with patch("pyOFTools.sets.meshSearch", return_value=mock_mesh_search):
            ...
            result = create_polyline_set(
                mesh=mock_mesh, name="polyline1", points=points, n_points=20
            )
    ...
    with patch("pyOFTools.sets.sampledSet") as mock_sampledSet:
        with patch("pyOFTools.sets.meshSearch", return_value=mock_mesh_search):
            with patch("pyOFTools.sets.Word") as mock_word:
                ...
                points = [[0, 0, 0], [1, 0, 0]]
                result = create_polyline_set(
                    mesh=mock_mesh, name="polyline1", points=points, n_points=10, axis="xyz"
                )
